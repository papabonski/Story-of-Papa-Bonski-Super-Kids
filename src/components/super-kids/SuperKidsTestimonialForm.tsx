...
export default function SuperKidsTestimonialForm({ customerName }: { customerName: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    setState("saving");
    const response = await fetch("/api/super-kids/testimonial", {
      method: "POST",
  ...
  if (state === "done") return <div className="rounded-3xl bg-emerald-50 p-6 text-emerald-950 ring-1 ring-emerald-200"><h2 className="text-xl font-extrabold">Terima kasih atas ulasan jujurnya.</h2><p className="mt-2 text-sm">Masukan Anda membantu Papa Bonski Super Kids menjadi lebih baik.</p></div>;

  return <form onSubmit={submit} className="space-y-5">
    <div><label className="font-extrabold">Pengalaman menggunakan Papa Bonski</label><div className="mt-2 flex gap-2" role="radiogroup" aria-label="Nilai pengalaman">{[1,2,3,4,5].map(n=><button key={n} type="button" onClick={()=>setRating(n)} aria-pressed={rating===n} className={`rounded-xl px-3 py-2 text-xl ${rating===n?"bg-amber-100 ring-2 ring-amber-400":"bg-surface"}`}>★</button>)}</div></div>
    <div><label htmlFor="review" className="font-extrabold">Ulasan jujur</label><textarea id="review" required minLength={20} maxLength={2000} rows={6} value={review} onChange={e=>setReview(e.target.value)} placeholder="Ceritakan pengalaman anak dan orang tua, bagian yang membantu, serta apa yang masih perlu diperbaiki." className="mt-2 w-full rounded-2xl border border-black/10 p-4"/><p className="mt-1 text-xs text-ink-soft">Tidak harus positif. Kritik dan saran tetap sangat kami hargai.</p></div>
