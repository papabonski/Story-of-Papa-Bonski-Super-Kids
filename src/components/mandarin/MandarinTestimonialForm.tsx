...
export default function MandarinTestimonialForm({ customerName }: { customerName: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    setState("saving");
    const response = await fetch("/api/mandarin/testimonial", {
      method: "POST",
  ...
  if (state === "done") return <div className="rounded-3xl bg-emerald-50 p-6 text-emerald-950 ring-1 ring-emerald-200"><h2 className="text-xl font-extrabold">Terima kasih atas ulasan jujurnya.</h2><p className="mt-2 text-sm">Masukan Anda membantu Papa Bonski Mandarin menjadi lebih baik.</p></div>;
