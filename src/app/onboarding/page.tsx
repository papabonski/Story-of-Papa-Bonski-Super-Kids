...
export default async function OnboardingPage() {
  ...
  if (!access) return <main className="min-h-[100dvh] bg-surface px-5 py-10 text-ink"><div className="mx-auto max-w-lg rounded-[2rem] bg-surface-card p-7 text-center shadow-xl ring-1 ring-black/[0.06]">
    ...
    <div className="mt-5 rounded-2xl bg-amber-50 p-4 text-left text-sm text-amber-900"><b>Yang bisa dilakukan:</b><br/>1. Pastikan email ini sama dengan email saat checkout OrderHero.<br/>2. Jika baru membayar, tunggu sebentar lalu muat ulang halaman.<br/>3. Jika tetap belum aktif, hubungi Papa Bonski dan sertakan bukti/order ID.</div>
  ...
  return <main className="min-h-[100dvh] bg-surface px-5 py-10 text-ink"><div className="mx-auto max-w-lg">
    <div className="rounded-[2rem] bg-surface-card p-7 text-center shadow-xl ring-1 ring-black/[0.06]">
      ...
      <p className="mt-3 text-sm leading-relaxed text-ink-soft">Pembelian sudah terhubung otomatis. Papa Bonski Super Kids siap digunakan.</p>
      <div className="mt-6 grid gap-3 text-left sm:grid-cols-2">
        <div className="rounded-2xl bg-white p-4 ring-1 ring-black/[0.05]"><div className="text-xs font-bold text-ink-faint">Customer ID</div><div className="mt-1 font-extrabold">{access.customerCode}</div></div>
        <div className="rounded-2xl bg-white p-4 ring-1 ring-black/[0.05]"><div className="text-xs font-bold text-ink-faint">Paket</div><div className="mt-1 font-extrabold">{access.planName || "Super Kids"}</div></div>
      </div>
