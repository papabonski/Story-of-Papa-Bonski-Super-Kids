...
export default async function OrderHeroIntegrationPage(){
  ...
  return <main className="min-h-[100dvh] bg-surface px-5 py-10 text-ink"><div className="mx-auto max-w-6xl">
    ...
    <p className="mt-4 text-xs font-extrabold uppercase tracking-[.18em] text-brand-primary">V5.5 Secure Operations</p>
    <h1 className="mt-2 text-3xl font-extrabold">OrderHero Purchase Activation</h1>
    <p className="mt-2 max-w-3xl text-sm text-ink-soft">Webhook URL, payload transaksi, dan mapping produk hanya terlihat setelah sesi Seller Center tervalidasi.</p>
    ...
    <section className="mt-6 rounded-2xl bg-surface-card p-6 ring-1 ring-black/[.06]"><h2 className="text-lg font-extrabold">Webhook URL</h2><div className="mt-3 break-all rounded-xl bg-black/5 p-4 font-mono text-sm">{webhookUrl}</div><p className="mt-3 text-xs text-ink-soft">Jangan membagikan URL ini. Jika memakai token URL, token tersebut adalah secret integrasi.</p></section>
    <section className="mt-6 rounded-2xl bg-surface-card p-6 ring-1 ring-black/[.06]"><h2 className="text-lg font-extrabold">Product Mapping</h2><p className="mt-1 text-sm text-ink-soft">Mapping product_id / nama produk / SKU OrderHero ke plan Papa Bonski.</p><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b"><th className="p-2">Match</th><th className="p-2">Value</th><th className="p-2">Product</th><th className="p-2">Plan</th></tr></thead><tbody>{mappings.map(m=><tr key={m.id} className="border-b border-black/5"><td className="p-2">{m.match_type}</td><td className="p-2 font-mono text-xs">{m.match_value}</td><td className="p-2">{m.product_sku}</td><td className="p-2">{m.plan_code}</td></tr>)}</tbody></table></div></section>
