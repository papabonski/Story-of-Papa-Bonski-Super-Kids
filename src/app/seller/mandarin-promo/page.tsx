...
export default async function MandarinPromoPage() {
  await requireSellerSession("/seller/mandarin-promo");
  const db = createSupabaseAdminClient();
  const now = new Date().toISOString();
  const [{ data: promo }, { data: claims }] = await Promise.all([
    db.from("promotions").select("name,quota,active,coupon_code").eq("key","mandarin-launch-50").maybeSingle(),
    db.from("promo_claims").select("id,slot_number,status,recipient_email,activated_at,feedback_due_at,feedback_submitted_at,reminder_count").eq("promotion_key","mandarin-launch-50").order("slot_number"),
  ]);
  ...
  return <main className="min-h-[100dvh] bg-surface px-5 py-10 text-ink"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-4 flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.16em] text-brand-primary">Promo Mandarin</p><h1 className="mt-2 text-3xl font-extrabold">50 pengguna awal</h1><p className="mt-2 text-sm text-ink-soft">Pantau kuota gratis dan ulasan yang sudah jatuh tempo.</p></div><span className={`rounded-full px-4 py-2 text-sm font-extrabold ${promo?.active?"bg-emerald-50 text-emerald-800":"bg-amber-50 text-amber-800"}`}>{promo?.active?"Aktif":"Belum aktif"}</span></div>
